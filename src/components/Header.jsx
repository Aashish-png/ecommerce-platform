  ...
  return (
    <header className="sticky top-0 bg-white shadow-md z-10">
      <div className="container mx-auto px-4 py-4 flex justify-between items-center">
       <Link  to="/" > <h1 className="text-2xl font-bold">E-Commerce Platform</h1></Link>
        <nav className='flex justify-center items-center  gap-1'>
          ...
          <Link to="/cart"  className=' relative'>  <img className=' w-11  h-11 '  src={CARTIMAGE} alt="cart"  /> 
            <span  className=' px-4 py-2 absolute top-0  left-2  rounded-full text-white    text-sm  font-bold'>{cart.length}</span></Link>
        </nav>
      </div>
    </header>
  );
